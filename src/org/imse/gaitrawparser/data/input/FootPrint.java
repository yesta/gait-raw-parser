...
public abstract class FootPrint {
	...
	private void calculateALRG() {
		...
		while (agLine == null) {
			for (int i = 0; i < innerPoints.size(); i++) {
				for (int j = i + 1; j < innerPoints.size(); j++) {
					...
					double diff = getDirectionDiff(line.getA(), getTargetDirectionAG());
					if (diff < minDiff && allPointsOnOneSideOfLine(line, takenPoints)) {
						minDiff = diff;
		...
		while (lrLine == null) {
			for (int i = 0; i < outerPoints.size(); i++) {
				for (int j = i + 1; j < outerPoints.size(); j++) {
					...
					double diff = getDirectionDiff(line.getA(), getTargetDirectionLR());
					if (diff < minDiff && allPointsOnOneSideOfLine(line, takenPoints)) {
						minDiff = diff;
		...
		/*
		 * a = new DoublePoint(normal.getP1()); g = new
		 * DoublePoint(normal.getP2());
		 * 
		 * l = new DoublePoint(lrLine.getP1()); r = new
		 * DoublePoint(lrLine.getP2());
		 */

		/*
		 * a = new DoublePoint(agLine.getP1()); l = new
		 * DoublePoint(lrLine.getP1()); g = new DoublePoint(agLine.getP2()); r =
		 * new DoublePoint(lrLine.getP2());
		 */
	}
	...
	private void calculateCNPE() {
		...
		DoubleLine cn = new DoubleLine(c, new DoublePoint(c.x + agNormal.x, c.y
				+ agNormal.y));
		DoubleLine ep = new DoubleLine(e, new DoublePoint(e.x + agNormal.x, e.y
				+ agNormal.y));
		...
		heelCenter = (new DoubleLine(a, n))
				.getIntersection(new DoubleLine(l, c));
	...
	private Line getHeelNormalThroughPoint(Line agLine, Point p) {
		...
		if (Math.signum(agLine.getA().x) != Math.signum(agLine.getA().y)) {
			...
			return new Line(p.x, p.y + 1, p.x + normalVector.x, p.y + 1
					+ normalVector.y);
		} else {
			// Linker oberer Eckpunkt
			return new Line(p.x, p.y, p.x + normalVector.x, p.y
					+ normalVector.y);
		}
	}
	...
	private Line getToeNormalThroughPoint(Line agLine, Point p) {
		...
		if (Math.signum(agLine.getA().x) != Math.signum(agLine.getA().y)) {
			// Rechter oberer
			return new Line(p.x + 1, p.y, p.x + 1 + normalVector.x, p.y
					+ normalVector.y);
		} else {
			// Rechter unterer
			return new Line(p.x + 1, p.y + 1, p.x + 1 + normalVector.x, p.y + 1
					+ normalVector.y);
		}
	}

	private double distance(Point p1, Point p2) {
		return Math.sqrt(Math.pow(((double) p2.x) - ((double) p1.x), 2)
				+ Math.pow(((double) p2.y) - ((double) p1.y), 2));
	}

	private double getDirectionDiff(Point vec1, Point vec2) {
		double cos = (vec1.x * vec2.x + vec1.y * vec2.y)
				/ (Math.sqrt(vec1.x * vec1.x + vec1.y * vec1.y) * Math
	...
	protected abstract Point getTargetDirectionAG();

	protected abstract Point getTargetDirectionLR();
